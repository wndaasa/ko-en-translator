...
> **데이터·가중치는 저장소에 포함하지 않습니다.** 코퍼스는 대용량·라이선스(AI Hub 재배포 금지) 이슈로 `prepare_data.py`/`prepare_aihub.py`로 재생성합니다. 학습 가중치는 Hugging Face **[Imsbee/ko-en-translator](https://huggingface.co/Imsbee/ko-en-translator)**로 배포했습니다(논문 문서모드 `doc-paper` + 범용 문장 `sentence-base`, apache-2.0). 데이터 준비 명령은 [CLAUDE.md](CLAUDE.md) 참고.
>
