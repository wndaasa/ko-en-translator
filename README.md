# 한↔영 번역 Transformer (직접 구현)

PyTorch로 Transformer를 **바닥부터 직접 구현**하는 학습 프로젝트입니다.
주 목적은 **한국어↔영어 번역 전용 소형 모델**이며, 부차적으로 간단한 대화도 가능하도록 합니다.

> 무거운 대화·문서 질의응답(RAG) 등은 별도의 대형 모델(30B+)이 담당하고,
...
(어텐션의 O(n²) 비용 때문에 소형 모델은 짧은 컨텍스트를 유지하고, 문서 길이는 애플리케이션 레벨에서 다룸).
